...
| C5 an oversized record-insert index collapses cardinal scorers and leaves rankers intact; ttl at the turnover recovers 80-90% | shadow ablation at 6 capacities | **missing → R2** | llm-d approximate mode is the default record-insert | scorers surviving 4x overgrowth on a trace | treatment_* + R2 run |
...
| C7 scale-in: the coldest member of a ranker fleet is its miss absorber and not free to remove | 3 victim rules x 3 policies | **missing → R2, else demoted to observation** | - | coldest removal matching random on a trace | scale_in_* |
