...
| C2 pure ranking + universal prefix locks onto one worker at any view freshness; a match threshold / load term / deep hash cap each cure it fully | controlled k-sweep, perfect view locks in identically; reproduced on the session workload (third family, cures identical, hit rises with u under every cure) | discovered on both traces (p99 1546s / p50 2017s) | sglang ships the threshold; corollary measured 5/9: a *stale* view breaks the lock-in (the accidental threshold) — the lock-in feeds on information | (held) | lockin + lockin_sessions |
...
| C5 an oversized record-insert index collapses cardinal scorers only where its ghosts are worker-differential AND persistently re-requested (a stationary differentiated hot set = the multi-tenant boilerplate regime, llm-d's own benchmark shape); chat sessions (mortal trunks) and universal-prefix agents (common-mode) are immune; rankers immune throughout; ttl at the turnover recovers 80-90% where it bites | zipf = the tenant regime: collapse | toolagent: null (common-mode); sessions: null (unmatchable ghosts) — both falsifiers fired and *scoped* the claim to its regime | llm-d approximate mode is the default record-insert, and llm-d's benchmark is exactly the vulnerable workload | a stationary differentiated workload where 4x overgrowth leaves scorers intact | treatment_* + both ablations |
...
| C8 no collapse under stale views: cache state is slow state (the thesis) | full staleness map on zipf AND sessions (nonstationary: fp slope ~2x as the cold-share theory predicts, orderings unchanged, no crossover) | both traces (staleness nearly irrelevant at trace cache sizes) | contra stale-queue load balancing (mitzenmacher); chwbl added as the never-reads-a-view control (flat at every age) | any crossover below blind within one turnover | staleness sweeps incl. _sessions + (C-track) real engine |
...
| 3 | stationary hot set: fixed zipf prefixes + poisson flatter staleness results | **measured (5/9)**: sessions double the fp-per-turnover slope (hybrid ~7% -> ~12%; dualmap steeper: deep keys promise mortal trunks) exactly as slope-=-cold-share predicts, and the thesis holds. the same stationarity is what makes record-insert overgrowth harmful (C5), so the bias cuts both ways and both directions are measured | done; zipf kept as the tenant-boilerplate regime, sessions as the chat regime |
| 4 | placeholder worker costs: knees, p99s, the shield's 110s are relative numbers | **bounded (5/9)**: x0.5/x2 sweep leaves every ordering, the no-crossover thesis, and the a/T_C shape intact; the exception is any fixed-weight cardinal score, whose absolute level shifts with cost because it mixes token units with queue units (hybrid fresh 0.569/0.597/0.660) — cardinal scorers are implicitly hardware-calibrated | C2 mac calibration for absolutes; scorer weights re-fit at calibration |
...
| 6 | seed/ci discipline: headline synthetic tables at 1-3 seeds without intervals | **closed (5/9)**: headline cells re-run at 10 seeds with 95% bootstrap cis (out/headline_ci.csv); every previously quoted median inside its interval; orderings interval-separated; trace runs labeled deterministic | seed_statistics available for any future table |
