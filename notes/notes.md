...
- cold replicas, what actually costs (5/9/26): the intuition "a cold cache drags the fleet down" was wrong on both scales measured. a ranker never routes matched work to an empty shelf, so it protects the fleet perfectly and *wastes the new worker entirely* (at trace scale: zero requests, ever, because the universal prefix means no order has a zero match). a scorer's load term uses the newcomer immediately and eats a small transient. so the scale-out problem is utilisation, not protection, and it is the router's property, not the cache's
    - shielding a newcomer by affinity ("give it only low-match work") is safe only when low-match work is scarce. on capacity-limited traffic low-match work is half of all traffic, and an affinity-only shield turns one cold worker into a self-inflicted hotspot (110s p99). any shield must be bounded by load first, affinity second, the chwbl (1+ε) shape
    - prewarming is the arm that works for every policy family, and for pure rankers it is the *only* one: a shelf must hold something before a ranker can ever choose it. warm-in times 0.16-0.46 T_C, floored by the transfer time
    - the autoscaler statement: budget scale-out in units of T_C (seconds at toy scale, minutes at trace scale), and check the router family before buying memory: scorers use it free, hash fleets nearly free, rankers only if you ship state to it

- what the audit week established (5/9/26): every falsifier that fired scoped a claim instead of killing it, and the scoping is the contribution
    - record-insert overgrowth harms a scorer only where ghosts are worker-differential *and* immortal in the request stream: the tenant-boilerplate regime (zipf-like). chat trunks are mortal (ghosts inert), universal prefixes are common-mode (score differences cancel). one workload property, stationarity, flatters snapshot staleness and damns record-insert, and both directions are measured now
    - nonstationarity doubles the staleness slope and changes no ordering: the law priced its own degradation (slope = cold share)
    - cost rescaling moves no ordering but shifts every fixed-weight cardinal score's level: a weighted score mixes units, so deployed scorers are implicitly calibrated to a hardware generation. ordinal use has no units
    - staleness breaks the lock-in: the stale photo is an accidental match threshold. the lock-in feeds on information, so removing information cures it; the deliberate threshold does the same job without the latency of the accident
    - and one data catch: the "two" mooncake agent traces are the same trace released twice. check your independence assumptions with cmp before counting workloads
