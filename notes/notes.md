...
- two diseases, two cures (2/9/26, late): a stale view can be wrong in two directions, and the error accounting tells them apart
    ...
    - the ranker clause (from the toolagent trace): an overlay is record-insert with a short memory, and record-insert plus a pure ranker herds on bursty shared-prefix traffic no matter how short the memory, because the burst is same-instant. so the blind-spot cure is for scorers with a load term; a pure ranker on real traffic keeps its blind spot (which was spreading its bursts) or takes a hash cap
