...
- predictions before the session-workload and cost-sensitivity runs (5/9/26, R3/R5, written before results): four runs launched against the artifact register
    - sessions staleness sweep: still no collapse to blind within a turnover, but a *steeper* fp-per-turnover slope than zipf's ~5% (churning trunks raise the cold share of promises; the theory says the slope is the cold share, so nonstationarity should show up exactly there). falsifier for the thesis if cache-aware crosses below blind within one turnover
